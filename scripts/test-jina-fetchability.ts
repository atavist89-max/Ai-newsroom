/**
 * Jina AI Reader Fetchability Test
 *
 * This script tests real Brave Search results against Jina AI Reader
to measure how many articles can actually be fetched in full text.
 *
 * Run with:
 *   BRAVE_API_KEY=xxx npx tsx scripts/test-jina-fetchability.ts
 */
...
async function jinaFetch(url: string): Promise<{
  text: string;
  status: TestResult["jinaStatus"];
  error?: string;
}> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status >= 500) return { text: "", status: "error", error: `HTTP ${res.status}` };
    ...
    if (
      ...
      lower.indexOf("comments") < 200 &&
      wordCount < 150
    ) {
      return { text, status: "garbage", error: "Likely comment section or nav page" };
    }

    return { text, status: "success" };
  } catch (err: any) {
    if (err.name === "AbortError") {
      return { text: "", status: "timeout", error: "15s timeout" };
    }
...
const TEST_CASES = [
  { countryCode: "SE", countryName: "Sweden", continent: "Europe", language: "Swedish", topics: ["politik", "ekonomi", "teknik"] },
  { countryCode: "GB", countryName: "United Kingdom", continent: "Europe", language: "English", topics: ["politics", "economy", "technology"] },
  { countryCode: "US", countryName: "United States", continent: "North America", language: "English", topics: ["politics", "economy", "technology"] },
  { countryCode: "DE", countryName: "Germany", continent: "Europe", language: "German", topics: ["politik", "wirtschaft", "technologie"] },
  { countryCode: "IN", countryName: "India", continent: "Asia", language: "English", topics: ["politics", "economy", "technology"] },
  { countryCode: "ZA", countryName: "South Africa", continent: "Africa", language: "English", topics: ["politics", "economy", "technology"] },
];
...
async function runTest() {
  ...
  for (const testCase of TEST_CASES) {
    for (const topic of testCase.topics) {
      ...
      try {
        ...
        const top5 = localResults.slice(0, 5);
        for (const r of top5) {
          const url = r.url;
          const source = new URL(url).hostname;
          console.log(`  Fetching Jina: ${source}`);
          ...
          results.push({
            ...
            jinaWordCount: jina.text ? jina.text.split(/\s+/).filter((w: string) => w.length > 0).length : 0,
            jinaPreview: jina.text ? jina.text.slice(0, 120).replace(/\n/g, " ") : "",
            errorMessage: jina.error,
          });
          console.log(`    → ${jina.status}${jina.error ? ` (${jina.error})` : ""}`);
      ...
      console.log(`\n🔍 ${testCase.countryName} CONTINENT — ${topic}`);
      try {
        const continentResults = await braveSearch(continentQuery, 10);
        const top5 = continentResults.slice(0, 5);
        for (const r of top5) {
          const url = r.url;
          const source = new URL(url).hostname;
          console.log(`  Fetching Jina: ${source}`);
  ...
  // Breakdown by source domain (top blocked)
  console.log("\n--- Top Blocked Domains ---");
  const blockedDomains: Record<string, number> = {};
  for (const r of results) {
    if (r.jinaStatus === "blocked" || r.jinaStatus === "too_short") {
      blockedDomains[r.source] = (blockedDomains[r.source] || 0) + 1;
    }
  }
  const sortedBlocked = Object.entries(blockedDomains).sort((a, b) => b[1] - a[1]).slice(0, 15);
  for (const [domain, count] of sortedBlocked) {
    console.log(`  ${domain.padEnd(30)} ${count}`);
  }

  // Word count distribution for successes
  console.log("\n--- Word Count Distribution (successes only) ---");
  ...
  console.log(`  Median: ${successes.sort((a, b) => a.jinaWordCount - b.jinaWordCount)[Math.floor(successes.length / 2)]?.jinaWordCount ?? 0} words`);
